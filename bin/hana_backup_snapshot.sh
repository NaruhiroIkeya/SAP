...
SSMPARAMDATAVOL="$Hostname-hdb-datavolumes"
SSMPARAMLOGVOL="$Hostname-hdb-logvolumes"
...
if [ "AWS" = $CLOUD ]; then
  ...
  instanceid=`aws ec2 describe-instances --filter "Name=tag:Name,Values=$Hostname" --region ap-northeast-1 | jq -r ".Reservations[].Instances[].InstanceId"`
  ...
  sidadm=$(ps -C sapstart -o user | awk 'NR ==3' | awk '{print $1}')
else
...
tag_mountinfo() {
  if [ "AWS" = $CLOUD ]; then
    ...
    do
      ...
      #add tag to snapshot
      aws ec2 create-tags --region $region --resource ${recent_snapshot_list_new[$i]} --tags Key=device_name,Value=${device_name_sorted[$i]} Key=hana_snapshot_id,Value=${SnapshotID} Key=comment,Value=${Comment}
...
#        aws ec2 delete-snapshot --region $region --snapshot-id ${recent_snapshot_list_new[$i]}
#        log "ERROR: Backup was not marked as successful in HANA backup catalog - delete EBS snapshot: ${recent_snapshot_list_new[$i]}"
#      done
#    elif [ "AZURE" = $CLOUD ]; then
#      return 0
#    else
